...
function harness(t: TestContext, columns?: number) {
...
test("a slow download renders progress and clears the line when done", (t) => {
  ...
  assert.match(
    writes[1],
    /\[===\|\.\.\.\.\.\.\.\.\] downloading kernel\.bin: 1\.0\/4\.0 MB \(25%\)/,
  );
  indicator.update("kernel.bin", "kernel.bin", 2 * MB, 4 * MB, false);
  tick(100);
  assert.match(
    writes[2],
    /\[======\/\.\.\.\.\.\] downloading kernel\.bin: 2\.0\/4\.0 MB \(50%\)/,
  );
  indicator.update("kernel.bin", "kernel.bin", 4 * MB, 4 * MB, true);
  assert.equal(writes.at(-1), "\r\x1b[K\x1b[1A\r\x1b[K");
});
...
test("concurrent downloads aggregate under a shared label", (t) => {
  ...
  tick(250);
  assert.match(writes[1], /downloading guest images: 2\.0\/8\.0 MB \(25%\)/);
...
test("received bytes beyond the expected total stay clamped", (t) => {
  ...
  assert.match(
    writes.at(-1)!,
    /\[============\] downloading root filesystem: 4\.0\/4\.0 MB \(100%\)/,
  );
});

test("progress stays on one row in a narrow terminal", (t) => {
  const { writes, indicator, tick } = harness(t, 30);
  indicator.update("disk-seed", "root filesystem", MB, 4 * MB, false);
  tick(250);
  const visible = writes.at(-1)!.replace(/\r|\x1b\[[0-9;]*[A-Za-z]/g, "");
  assert.equal(visible, "[===|........] 25%");
  assert.ok(visible.length < 30);
});

test("the marker animates while byte progress is unchanged", (t) => {
  const { writes, indicator, tick } = harness(t, 30);
  indicator.update("kernel.bin", "kernel.bin", MB, 4 * MB, false);
  tick(250);
  tick(100);
  assert.match(writes.at(-1)!, /\[===\/\.\.\.\.\.\.\.\.\] 25%/);
  tick(100);
  assert.match(writes.at(-1)!, /\[===-\.\.\.\.\.\.\.\.\] 25%/);
});
